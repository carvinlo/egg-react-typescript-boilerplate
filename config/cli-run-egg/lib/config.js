...
class Config {
	constructor(config) {
		...
		this.sourceMapLoaders = ['css-loader', 'sass-loader', 'less-loader', 'stylus-loader'];
	...
	mergeWebpackConfig(sourceWebpackConfig = {}, newWebpackConfig = {}) {
		...
		sourceKeys.forEach(key => {
			...
			if (utils.isString(sourceValue) || utils.isBoolean(sourceValue)) {
				...
				utils.isObject(sourceValue) ||
				Array.isArray(sourceValue) ||
				utils.isObject(newValue) ||
	...
	isUse(name, range = 'plugin') {
		...
		const configInfo = this.utils.isObject(name)
			? name
			: range === 'plugin'
				? this.getPluginByName(name)
				: this.getLoaderByName(name);
		return (
			configInfo && this.isType(configInfo.type) && this.isEnv(configInfo.env) && this.isEnable(configInfo.enable)
		);
	}
	...
	isWebpackPlugin(plugin) {
		return (
			this.utils.isObject(plugin) &&
			plugin.constructor &&
			plugin.constructor.prototype &&
			plugin.constructor.prototype.apply
		);
	}
	...
	createFileName(config) {
		if ((config.hash && this.utils.isTrue(config.fileHash)) || config.fileHash) {
			...
			this.setOutputFileChunkName(
				utils.assetsPath(this.prefix, `js/chunk/[name].[chunkhash:${config.hashLength}].js`)
			);
		} else {
	...
	createCssName(config) {
		if ((config.hash && this.utils.isTrue(config.cssHash)) || config.cssHash) {
			this.webpackInfo.cssName = utils.assetsPath(
				this.prefix,
				`css/[name].[contenthash:${config.hashLength}].css`
			);
			this.webpackInfo.cssChunkName = utils.assetsPath(
				this.prefix,
				`css/[id].[contenthash:${config.hashLength}].css`
			);
		} else {
	...
	createMediaName(config) {
		if ((config.hash && this.utils.isTrue(config.mediaHash)) || config.mediaHash) {
			this.webpackInfo.mediaName = utils.assetsPath(
				this.prefix,
				`media/[name].[hash:${config.hashLength}].[ext]`
			);
		} else {
